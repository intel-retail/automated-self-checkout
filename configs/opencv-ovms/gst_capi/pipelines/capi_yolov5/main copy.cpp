...
public:
    ...
    virtual const std::tuple<double, double, double, double> calc_bounding_box(size_t col, size_t row, float raw_x,
                                                                         float raw_y, float raw_w, float raw_h,
                                                                         size_t side_w, size_t side_h, int mask_0,
                                                                         size_t bbox_cell_num) =0;

    virtual const void convert_to_relative_coords(float &x_min, float &y_min, float &w, float &h) =0;

    virtual const size_t entry_index(size_t side_square, size_t location, size_t entry)=0;

    virtual const std::vector<float> softmax(const float *arr, size_t size, size_t common_offset, size_t side_square) = 0;
...
public:
    ...
    const std::tuple<double, double, double, double> calc_bounding_box(size_t col, size_t row, float raw_x,
                                                                         float raw_y, float raw_w, float raw_h,
                                                                         size_t side_w, size_t side_h, int mask_0,
                                                                         size_t bbox_cell_num) {
        ...
        float x = static_cast<float>(col + raw_x) / side_w * _window_width;
        float y = static_cast<float>(row + raw_y) / side_h * _window_height;

        const size_t anchor_index = (2 * mask_0) + (2 * bbox_cell_num);
        if (sizeof(anchors) <= anchor_index + 1)
            throw std::runtime_error("Invalid anchor index: out of array bounds");
        float w = std::exp(raw_w) * anchors[anchor_index];
        float h = std::exp(raw_h) * anchors[anchor_index + 1];

        convert_to_relative_coords(x, y, w, h);
        return {x, y, w, h};
    }

    const void convert_to_relative_coords(float &x_min, float &y_min, float &w, float &h) {
        x_min -= w / 2;
        y_min -= h / 2;
        x_min /= _window_width;
        w /= _window_width;
        y_min /= _window_height;
        h /= _window_height;
    }

    const size_t entry_index(size_t side_square, size_t location, size_t entry) {
        ...
        return side_square * (bbox_cell_num * (classes + 5) + entry) + loc;
    }

    const std::vector<float> softmax(const float *arr, size_t size, size_t common_offset, size_t side_square) {
    ...
    void postprocess(const int64_t* output_shape, const void* voutputData, const size_t *input_shape, const size_t bytesize, const uint32_t dimCount, std::vector<DetectedResult> &detectedResults)
    {
        ...
        const int &mask= masks.at(std::min(sideW, sideH));
        printf("entriesNum %d, regionNum: %d regionCoordsCount: %d: \n", entriesNum, regionNum, regionCoordsCount);
        ...
        for (int i = 0; i < entriesNum; ++i) {
            ...
            for (int n = 0; n < regionNum; ++n) {
                const size_t common_offset = n*entriesNum+i;
                const size_t bbox_conf_index = entry_index(entriesNum, common_offset, regionCoordsCount);
                const size_t bbox_index = entry_index(entriesNum, common_offset, 0);

                float bbox_conf = outData[bbox_conf_index];
                // if (_output_sigmoid_activation)
                ...
                bool _use_softmax = true;
                if (_use_softmax) {
                    const auto cls_confs = softmax(outData, classes, common_offset, entriesNum);

                    for (size_t bbox_class_id = 0; bbox_class_id < classes; ++bbox_class_id) {
                    ...
                    for (size_t bbox_class_id = 0; bbox_class_id < classes; ++bbox_class_id) {
                        const size_t class_index = entry_index(entriesNum, common_offset, 5 + bbox_class_id);
                        const float bbox_class_prob = outData[class_index];
                ...
                const float raw_x = outData[bbox_index + 0 * entriesNum];
                const float raw_y = outData[bbox_index + 1 * entriesNum];
                const float raw_w = outData[bbox_index + 2 * entriesNum];
                const float raw_h = outData[bbox_index + 3 * entriesNum];

                auto [x, y, w, h] =
                    calc_bounding_box(col, row, raw_x, raw_y, raw_w, raw_h, sideW, sideH, mask, n);
                ...
                obj.classId = bbox_class.first;
                detectedResults.emplace_back(obj);
