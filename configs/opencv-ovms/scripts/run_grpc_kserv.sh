...
python3 /scripts/grpc_python.py --input_src 'rtsp://127.0.0.1:8554/camera_0' --grpc_address 127.0.0.1 --grpc_port $GRPC_PORT --model_name  instance-segmentation-security-1040 2>&1  | tee >/tmp/results/r$cid_count.log >(stdbuf -oL sed -n -e 's/^.*fps: //p' | stdbuf -oL cut -d , -f 1 > /tmp/results/pipeline$cid_count.log)